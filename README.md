# FingerEye

**Continuous and Unified Vision-Tactile Sensing for Dexterous Manipulation**

[![arXiv](https://img.shields.io/badge/arXiv-FingerEye-F5F5F7?style=flat-square&logo=arxiv&logoColor=111111&labelColor=F5F5F7&color=E8E8ED)](https://arxiv.org/pdf/2604.20689)
[![Project Website](https://img.shields.io/badge/Project-Website-F5F5F7?style=flat-square&logo=safari&logoColor=111111&labelColor=F5F5F7&color=E8E8ED)](https://nus-lins-lab.github.io/FingerEyeWeb/)
[![Hardware Guide](https://img.shields.io/badge/Hardware-Guide-F5F5F7?style=flat-square&logo=googledocs&logoColor=111111&labelColor=F5F5F7&color=E8E8ED)](https://docs.google.com/document/d/1jIsauCUubnpsnRZiPJ1yBQh4KIW-sTdFwxu0cdAxOfM/edit?tab=t.0)

## Contents

| Subdirectory | Description | Readme |
| --- | --- | --- |
| `FingerEyePolicy/` | Policy architectures, preprocessing, and offline training | [Open](FingerEyePolicy/README.md) |
| `FingerEyeRW/` | Real-world setup, teleoperation, data collection, and deployment | [Open](FingerEyeRW/README.md) |
| `FingerEyeLab/` | Simulation environments, digital twin, and sim data generation | [Open](FingerEyeLab/README.md) |

## Citation

```bibtex
@misc{xu2026fingereyecontinuousunifiedvisiontactile,
    title={FingerEye: Continuous and Unified Vision-Tactile Sensing for Dexterous Manipulation},
    author={Zhixuan Xu and Yichen Li and Xuanye Wu and Tianyu Qiu and Lin Shao},
    year={2026},
    eprint={2604.20689},
    archivePrefix={arXiv},
    primaryClass={cs.RO},
    url={https://arxiv.org/abs/2604.20689},
}
```

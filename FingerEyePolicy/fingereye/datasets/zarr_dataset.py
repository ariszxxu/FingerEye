'''
The dataset structure is as follows: (some of the keys are optional)
/
 ├── data
 │   ├── obs
 │   │    ├── images: (n_total_steps, n_camera, 3, h, w) | uint8 | RGB | [0, 255]
 │   │    ├── depths: (n_total_steps, n_camera, 1, h, w) | float32 | in meter | >= 0
 │   │    ├── xyz_images: (n_total_steps, n_camera, 3, h, w) | float32 | in meter
 │   │    ├── point_clouds: (n_total_steps, 3, n_pc_points) | float32 | in meter
 │   │    ├── point_colors: (n_total_steps, 3, n_pc_points) | uint8 | [0, 255]
 │   │    ├── point_is_robot_mask: (n_total_steps, 1, n_pc_points) | uint8 | True if the point belongs to robot
 │   │    ├── robot_masks: (n_total_steps, 1, h, w) | uint8 | True if the pixel belongs to robot
 │   │    ├── ..._qpos: e.g."robot0_gripper_qpos", "robot0_all_qpos" | (n_total_steps, n_robot_or_gripper_dof) | float32 | in radians or meters, joint orders should be described in meta
 │   │    ├── ..._qvel: e.g."robot0_gripper_qvel", "robot0_all_qvel" | (n_total_steps, n_robot_or_gripper_dof) | float32 | in rad/s or m/s, joint orders should be described in meta
 │   │    ├── ..._pos: e.g."robot0_eef_pos" | (n_total_steps, 3) | float32 | in meter, X_WorldEE[:3, 3]
 │   │    ├── ..._lin_vel: e.g."robot0_eef_lin_vel" | (n_total_steps, 3) | float32 | in m/s, vector described in world frame
 │   │    ├── ..._quat: e.g."robot0_eef_quat" | (n_total_steps, 4) | float32 | in wxyz, X_WorldEE[:3, :3] -> wxyz quat
 │   │    ├── ..._ang_vel: e.g."robot0_eef_ang_vel" | (n_total_steps, 3) | float32 | in rad/s, axisang described in world frame
 ...
 │   │    ├── target_..._qpos: e.g."target_all_qpos" | (n_total_steps, n_dof) | float32 | a PD-controller target, in radians or meters, joint orders should be described in meta
 │   │    └── original_actions: (n_total_steps, d_action) | float32 | action vectors in orginal simulation envs dataset
 │   └── states: (n_total_steps, d_state) | float32 | state vectors in original simulation envs dataset
 └── meta
     ...
     ├── joint_name_list: (n_joint,) | str | an ordered list containing the joint names to describe the qpos & qvel...
     ├── camera_meta
     ...
     ├── model_files: (n_episode,) | str | each model file completely defines an mujoco/... env, which may give us access to more info.
     └── ...
'''
import zarr
import numpy as np
...
class ZarrDataset:
    ...
    def validate_shape_and_type(self, key: str, array: np.ndarray):
        ...
        if "images" in key and "xyz_images" not in key:
            ...
            assert array.dtype in [np.float32, np.uint8], f"{key} must be of type float32 or uint8."
            ...
            assert array.dtype == np.uint8, f"{key} must be of type uint8."
        elif "qpos" in key or "qvel" in key or "pos" in key or "quat" in key or "lin_vel" in key or "ang_vel" in key or "states" in key or "original_actions" in key:

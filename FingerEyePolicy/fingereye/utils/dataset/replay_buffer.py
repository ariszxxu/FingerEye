...
def copy_meta(src_root):
    """
    Recursively copy all layers of the 'meta' group from the source Zarr root.

    Parameters:
    - src_root: The source Zarr group to copy from.

    Returns:
    - A dictionary containing all 'meta' data.
    """
    def copy_group(group):
        """
        Recursively traverse the Zarr group and copy its data to a dictionary.

        Parameters:
        - group: The Zarr group to traverse.

        Returns:
        - A nested dictionary containing the group's data.
        """
        meta_dict = {}
        for key, item in group.items():
            if isinstance(item, zarr.Group):
                # If the item is a group, recurse into it
                meta_dict[key] = copy_group(item)
            else:
                # If the item is an array, check if it's a scalar
                if item.shape == ():  # Scalar array
                    meta_dict[key] = item[()]  # Access scalar values
                else:
                    meta_dict[key] = item[:]  # Access non-scalar array data
        return meta_dict

    # Start from the 'meta' group
    return copy_group(src_root['meta'])
...
def get_chunk_size(array: np.ndarray, target_chunk_size_bytes=2 * 1024 * 1024):
    """
    ...
    if array.ndim >= 2:
        ...
        target_elements = target_chunk_size_bytes // item_size
        chunk_shape = [1] + list(shape[1:])
        chunk_shape[0] = max(1, target_elements // np.prod(shape[1:]))
    ...
    def copy_from_store(
        ...
        **kwargs,
        ...
        if store is None:
            # numpy backend
            meta = dict()
            meta = copy_meta(src_root)
            def copy_key(key):
                return key, src_root["data"][key][:]
            with ThreadPoolExecutor(max_workers=16) as executor:
                results = list(executor.map(copy_key, keys))
            data = dict(results)
